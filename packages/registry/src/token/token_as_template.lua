...
mod.getInfo                   = function(msg)
    ...
    local replyTags = {
        ...
        TransferRestricted = tostring(AllowedRecipients ~= nil)
    }
...
mod.transfer                  = function(msg)
    ...
    -- Instead of an assert, reply with an error for non-whitelisted recipients
    if not _transferAllowed(msg.Recipient) then
        msg.reply({
            Action = 'Transfer-Error',
            ['Message-Id'] = msg.Id,
            Error = 'Transfer not allowed to Recipient'
        })
        return
    end
...
function mod.addAllowedRecipient(msg)
    assertOwner(msg)
    assert(type(msg.Recipient) == 'string', 'Recipient address is required!')

    AllowedRecipients = AllowedRecipients or {}
    if not recipientExists(AllowedRecipients, msg.Recipient) then
        table.insert(AllowedRecipients, msg.Recipient)
        print("Added " .. msg.Recipient)
    else
        print("Recipient already allowed")
    end
end

function mod.removeAllowedRecipient(msg)
    assertOwner(msg)
    assert(type(msg.Recipient) == 'string', 'Recipient address is required!')

    if AllowedRecipients then
        for i, addr in ipairs(AllowedRecipients) do
            if addr == msg.Recipient then
                table.remove(AllowedRecipients, i)
                print("Removed " .. msg.Recipient)
                return
            end
        end
    end
...
function mod.getAllowedRecipients(msg)
    assertOwner(msg)

    local replyData = AllowedRecipients or {}
    local replyTags = {
        Ticker = Ticker,
        Recipients = replyData
    }
    return mod.sendReply(msg, replyData, replyTags)
end
...
Handlers.add('addAllowedRecipient', { Action = "Add-Allowed-Recipient" },
    token_lib.addAllowedRecipient
)

Handlers.add('getAllowedRecipients', { Action = "Get-Allowed-Recipients" },
    token_lib.getAllowedRecipients
)

Handlers.add('removeAllowedRecipient', { Action = "Remove-Allowed-Recipient" },
    token_lib.removeAllowedRecipient
)
