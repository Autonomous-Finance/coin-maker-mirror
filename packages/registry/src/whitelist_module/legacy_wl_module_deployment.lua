do
    local _ENV = _ENV
    package.preload["token.token_lib"] = function(...)
        local arg   = _G.arg;
        ...
        local function assertOwner(msg)
            assert(msg.From == Owner, 'Only the Owner can call!')
        end

        -- Check if a sender address exists in a list
        local function senderExists(list, sender)
        ...
        -- Check if transferring to a sender is allowed
        local function _transferAllowed(sender)
            if AllowedSenders ~= nil then
                return senderExists(AllowedSenders, sender)
            end
            return true
        end
        ...
        mod.transfer                  = function(msg)
            ...
            -- Instead of an assert, reply with an error for non-whitelisted senders
            if not _transferAllowed(msg.From) then
                msg.reply({
                    Action = 'Transfer-Error',
                    ['Message-Id'] = msg.Id,
                    Error = 'Transfer not allowed: not whitelisted'
                })
                return
            end
        ...
        function mod.addAllowedSender(msg)
            assertOwner(msg)
            assert(type(msg.Address) == 'string', 'Address is required!')

            AllowedSenders = AllowedSenders or {}
            if not senderExists(AllowedSenders, msg.Address) then
                table.insert(AllowedSenders, msg.Address)
                print("Added " .. msg.Address)
            else
                print("Address already allowed")
            end
        end

        function mod.addAllowedSenderBatch(msg)
            assertOwner(msg)
            assert(type(msg.Addresses) == 'table', 'Addresses is required!')

            AllowedSenders = AllowedSenders or {}
            for _, addr in ipairs(msg.Addresses) do
                if not senderExists(AllowedSenders, addr) then
                    table.insert(AllowedSenders, addr)
                    print("Added " .. addr)
                else
                    print("Address already allowed")
                end
        ...
        function mod.removeAllowedSender(msg)
            assertOwner(msg)
            assert(type(msg.Address) == 'string', 'Address address is required!')

            if AllowedSenders then
                for i, addr in ipairs(AllowedSenders) do
                    if addr == msg.Address then
                        table.remove(AllowedSenders, i)
                        print("Removed " .. msg.Address)
                        return
                    end
                end
            end
        ...
        function mod.setTransferRestrictions(msg)
            assertOwner(msg)
            local enabled = parseBool(msg.Enabled)

            if enabled then
                AllowedSenders = {}
                print("Transfer restrictions enabled")
            else
                AllowedSenders = nil
                print("Transfer restrictions disabled")
            end
        end

        function mod.getAllowedSenders(msg)
            assertOwner(msg)

            local replyData = json.encode(AllowedSenders or {})
            local replyTags = {
                Ticker = Ticker,
                Senders = #AllowedSenders or 0
            }
            return mod.sendReply(msg, replyData, replyTags)
        end
...
Denomination      = 18
Balances          = Balances or { Owner = "1000000000000000000000000000" }
TotalSupply       = TotalSupply or "1000000000000000000000000000"
Vesting           = Vesting or {}

Name              = "YetAnotherFakeToken"
Ticker            = "YAFT"
Logo              = ""
